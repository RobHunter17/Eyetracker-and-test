...
# create keyboard_start object
keyboard_start = libinput.Keyboard(keylist = ['s'], timeout = None)
# create record file of trials
log = liblog.Logfile()
log.write(["trial#", "trialtype", "EndPos ", "latency ", "correct"])

# create screens where tests start
inscreen = libscreen.Screen()
inscreen.draw_text(text="When you see a cross, look at it and press s. Then make an eye movement to the black circle when it appears.\n\n(press s to start)", fontsize=24)
fixscreen = libscreen.Screen()
fixscreen.draw_fixation(fixtype='cross',pw=3)
# this is where the targets will appear on the screen
targetscreens = {}
...
disp.fill(inscreen)
disp.show()
keyboard_start.get_key()

# run any number of trials
for trialnr in range(1,6):
    # prepare trial
